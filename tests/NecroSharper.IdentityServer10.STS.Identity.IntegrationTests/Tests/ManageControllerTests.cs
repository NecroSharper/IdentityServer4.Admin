...
namespace NecroSharper.IdentityServer10.STS.Identity.IntegrationTests.Tests
{
    public class ManageControllerTests : BaseClassFixture
    {
        ...
        public async Task AuthorizeUserCanAccessManageViews()
        {
            ...
            {
                // Act
                var response = await Client.GetAsync($"/Manage/{route}", TestContext.Current.CancellationToken);
        ...
        public async Task UserIsAbleToUpdateProfile()
        {
            ...
            var manageResponse = await Client.GetAsync(manageAction, TestContext.Current.CancellationToken);
            ...
            var responseMessage = await Client.SendAsync(requestWithIdentityCookie, TestContext.Current.CancellationToken);
